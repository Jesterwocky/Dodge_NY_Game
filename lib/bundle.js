...
/***/ function(module, exports, __webpack_require__) {
	...
	DodgeNyGame.prototype.spawnPerson = function(obstacles) {
	  this.people.push(new MovingPerson({
	    ...
	    drift:          Math.floor(Math.random() * (5 - 2) + 2),
...
/***/ function(module, exports, __webpack_require__) {
	...
	MovingPerson.prototype.getVectorsToAvoidObstacle = function(collidedObstacles, vector) {
	  ...
	  if (horizontalBarrier !== undefined) {
	    if (vector[1] >= 0) {
	      let distanceToTravel = this.distance(horizontalBarrier[0], horizontalBarrier[1]);
	
	      let vectorsToAdd = [];
	      if (vector[0] >= 0) {
	        vectorsToAdd = [[1, -1], [1, 1]];
	        distanceToTravel = this.distance(this.center, horizontalBarrier[1]);
	      }
	
	      else {
	        vectorsToAdd = [[-1, -1], [-1, 1]];
	        distanceToTravel = this.distance(this.center, horizontalBarrier[0]);
	      }
	      ...
	      while (distanceTravelled <= distanceToTravel) {
	        vectors.push(vectorsToAdd);
	        distanceTravelled += this.distance(this.center,
	          [
	            this.center[0] + vectorsToAdd[0][0] + vectorsToAdd[1][0],
	            this.center[1] + vectorsToAdd[0][1] + vectorsToAdd[1][1]
	          ]);
	      }
	
	      this.temporaryVectors = [];
	      this.temporaryVectors.push(vectors);
	    }
	  }
	...
	MovingPerson.prototype.move = function() {
	  ...
	  let prospectiveNewPoint = [
	    ...
	    (this.center[1] + movement[1]) % FIELDHEIGHT
	  ];
	
	  if (vectorSource != this.temporaryVectors) {
	  ...
	  if (collidedObstacles.length > 0) {
	    this.getVectorsToAvoidObstacle(collidedObstacles, movement);
	  }
