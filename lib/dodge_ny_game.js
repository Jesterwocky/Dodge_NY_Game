...
const MovingPerson = require('./obstacle/moving_obstacle/moving_obstacle.js');
...
DodgeNyGame.prototype.spawnMovingObstacle = function() {
  ...
  options.speed = Math.ceil(Math.random() * 5);
  options.drift = Math.floor(Math.random() * 3);
  options.driftDirection = Math.random() <= 0.5 ? -1 : 1;
  options.canCrossStreet = Math.random() <= 0.9 ? false : true;
